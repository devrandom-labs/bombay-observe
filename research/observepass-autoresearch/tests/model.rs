...
//! because it deterministically hits FINDING-001, which is preserved
...
impl ModelState {
    ...
    fn register(&mut self, key: u8) -> bool {
        ...
        self.keys.insert(key, KeyState::Live { epoch, outcome: None });
        true
    }
    ...
    match op {
        Op::Register(key) => {
            ...
            if campaign.model.register(key) {
                let subject = result.unwrap_or_else(|e| panic!("model vacant but register failed: {e}"));
                campaign.harness.subjects.insert(key, subject);
            } else {
                assert!(result.is_err(), "model live but register succeeded for key {key}");
            }
        }
        Op::Complete(key) => {
            ...
            campaign
                .model
                .keys
                .insert(key, KeyState::Live { epoch, outcome: Some(value) });
        ...
        Op::Observe(key) => {
            ...
            match campaign.model.live(key) {
                Some((epoch, _)) => {
                    let observation = result.unwrap_or_else(|e| panic!("model live but observe failed: {e}"));
                    let id = campaign.harness.next_handle;
                    campaign.harness.next_handle += 1;
                    campaign.harness.observations.insert(
                        id,
                        ObsHandle { observation, key, epoch },
                    );
                }
                None => assert!(result.is_err(), "model vacant but observe succeeded for key {key}"),
            }
        }
        Op::TryGet(id) => {
        ...
        Op::RegisterWaker(id) => {
            ...
            let completed = campaign.model.outcome_of(handle.key, handle.epoch).is_some();
            assert_eq!(
            ...
            if !completed {
                campaign.registrations.push((handle.key, handle.epoch, probe));
            }
        }
        Op::IntoOutcome(id) => {
        ...
        Op::Retire(key) => {
            ...
            campaign.harness.subjects.remove(&key).expect("model live => subject handle");
