...
use bombay_observe_tests::probe::{CountWake, DropProbe};
        ...
        match op {
            ...
            'O' => {
                ...
                match model.live {
                    ...
                    None => assert!(
                        result.is_err(),
                        "{history:?}: observe while vacant succeeded"
                    ),
                }
            }
            'X' => {
    ...
    for completed in [false, true] {
        for (order, permutation) in permutations.iter().enumerate() {
            ...
            if completed {
                ...
                assert!(
                    bombay_observe_tests::probe::poll_once(
        ...
        for victim in (1..=3_u8).rev() {
            if subset & (1 << (victim - 1)) != 0 {
                ...
                assert_eq!(
                    space
                        .observe(&victim)
                        .expect("re-registered key observable")
                        .try_get(),
                    None,
    ...
    for polls in 0..=2_u8 {
        for complete_after_poll in [false, true] {
            for cancel in [false, true] {
                ...
                for _ in 0..polls {
                    let poll = bombay_observe_tests::probe::poll_once(future.as_mut(), &waker);
                    assert_eq!(
                        poll.is_ready(),
                        !complete_after_poll,
