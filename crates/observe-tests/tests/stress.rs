...
use bombay_observe_tests::probe::{CountWake, DropProbe, ThreadWake};
...
fn stress_reentrant_wake_drops_observation() {
    ...
    struct Reentrant {
        victim: std::sync::Mutex<Option<observe::Observation<u64>>>,
        fires: AtomicUsize,
    }
...
fn stress_reentrant_wake_reregistration_no_loop() {
    ...
    struct ReRegister {
        obs: std::sync::Mutex<Option<observe::Observation<u64>>>,
        fires: AtomicUsize,
    }
